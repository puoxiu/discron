...
func (s *Script) Insert() (insertId int, err error) {
	err = dbclient.GetMysqlDB().Table(CronixScriptTableName).Create(s).Error
	if err == nil {
		insertId = s.ID
	}
	return
...
func (s *Script) Update() error {
	return dbclient.GetMysqlDB().Table(CronixScriptTableName).Updates(s).Error
}

func (s *Script) Delete() error {
	return dbclient.GetMysqlDB().Exec(fmt.Sprintf("delete from %s where id = ?", CronixScriptTableName), s.ID).Error
}

func (s *Script) FindById() error {
	return dbclient.GetMysqlDB().Table(CronixScriptTableName).Where("id = ? ", s.ID).First(s).Error
}

func (s *Script) TableName() string {
	return CronixScriptTableName
}
