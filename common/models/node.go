...
// register to /Cronix/node/<node_uuid>/
type Node struct {
...
func (n *Node) Insert() (insertId int, err error) {
	err = dbclient.GetMysqlDB().Table(CronixNodeTableName).Create(n).Error
	if err == nil {
		insertId = n.ID
	}
	return
...
func (n *Node) Update() error {
	return dbclient.GetMysqlDB().Table(CronixNodeTableName).Updates(n).Error
}

func (n *Node) Delete() error {
	return dbclient.GetMysqlDB().Exec(fmt.Sprintf("delete from %s where uuid = ?", CronixNodeTableName), n.UUID).Error
}

func (n *Node) FindByUUID() error {
	return dbclient.GetMysqlDB().Table(CronixNodeTableName).Where("uuid = ? ", n.UUID).First(n).Error
}

func (n *Node) TableName() string {
	return CronixNodeTableName
}
