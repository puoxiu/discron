...
func (u *User) Update() error {
	return dbclient.GetMysqlDB().Table(CronixUserTableName).Updates(u).Error
}

func (u *User) Delete() error {
	return dbclient.GetMysqlDB().Exec(fmt.Sprintf("delete from %s where id = ?", CronixUserTableName), u.ID).Error
}

func (u *User) Insert() (insertId int, err error) {
	err = dbclient.GetMysqlDB().Table(CronixUserTableName).Create(u).Error
	if err == nil {
		insertId = u.ID
	}
	return
...
func (u *User) FindById() error {
	return dbclient.GetMysqlDB().Table(CronixUserTableName).Select("id", "username", "email", "role", "created", "updated").Where("id = ? ", u.ID).First(u).Error
}

func (u *User) TableName() string {
	return CronixUserTableName
}
