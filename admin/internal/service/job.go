...
func (j *JobService) GetTodayJobExcCount(success int) (int64, error) {
	db := dbclient.GetMysqlDB().Table(models.CronixJobLogTableName).Where("start_time > ? and end_time!=0 and success = ?", utils.GetTodayUnix(), success)
	var total int64
	err := db.Count(&total).Error
	if err != nil {
		return 0, err
...
func (j *JobService) GetJobExcCount(start, end int64, success int) ([]resp.DateCount, error) {
	...
	db := dbclient.GetMysqlDB().Table(models.CronixJobLogTableName).Select("FROM_UNIXTIME( start_time, '%Y-%m-%d' ) AS date", "COUNT( * ) AS count ").Group("date").Order("date ASC").Where("start_time > ? and start_time<?  and end_time!=0 and success = ?", start, end, success)
...
func (j *JobService) GetNotAssignedJob() (jobs []models.Job, err error) {
	err = dbclient.GetMysqlDB().Table(models.CronixJobTableName).Where("status = ?", models.JobStatusNotAssigned).Find(&jobs).Error
	return
}
...
func cleanupLogs(expirationTime int64) error {
	sql := fmt.Sprintf("delete from %s where start_time < ?", models.CronixJobLogTableName)
